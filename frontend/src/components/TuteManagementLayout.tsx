...
import {
    LayoutDashboard, Inbox, CheckCircle, FileText,
    Menu, GraduationCap, LogOut, ChevronDown,
} from "lucide-react";

const sidebarLinks = [
    { label: "Dashboard", icon: LayoutDashboard, path: "/tute-management/dashboard" },
    { label: "Incoming Requests", icon: Inbox, path: "/tute-management/incoming" },
    { label: "Active Requests", icon: CheckCircle, path: "/tute-management/active" },
    { label: "Delivery Records", icon: FileText, path: "/tute-management/delivery-records" },
];

export default function TuteManagementLayout() {
    ...
    useEffect(() => {
        // Check the shared auth role - TUTOR role is used for tute management
        const role = localStorage.getItem("authRole");
        if (role !== "TUTOR" && role !== "ADMIN" && role !== "TEACHER") {
            navigate("/signin");
        }
    }, [navigate]);
        ...
        <div className="min-h-screen bg-background flex">
            <aside className={`fixed inset-y-0 left-0 z-40 flex flex-col border-r border-border bg-sidebar transition-all duration-300 ${sidebarOpen ? "w-60" : "w-16"}`}>
                <div className="flex items-center gap-2 px-4 py-5 border-b border-sidebar-border">
                    ...
                    {sidebarOpen && <span className="font-display text-lg font-bold text-sidebar-foreground">Tute Management</span>}
                </div>
                    ...
                    <div className="flex items-center gap-2">
                        <div className="relative">
                            <button onClick={() => setProfileOpen(!profileOpen)}
                                ...
                                <span className="text-sm font-medium text-foreground hidden sm:block">{localStorage.getItem("authName") || "Tutor"}</span>
